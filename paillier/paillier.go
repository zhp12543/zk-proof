...
func GenerateKeyPair(modulusBitLen int, timeout time.Duration, optionalConcurrency ...int) (privateKey *PrivateKey, publicKey *PublicKey, err error) {
	...
	var P, Q, N *big.Int
	{
		tmp := new(big.Int)
		for {
			sgps, err := prime.GetRandomSafePrimesConcurrent(modulusBitLen/2, 2, timeout, concurrency)
			if err != nil {
				return nil, nil, err
			}
...
func (publicKey *PublicKey) EncryptAndReturnRandomness(m *big.Int) (c *big.Int, x *big.Int, err error) {
	...
	x = curve.GetRandomPositiveRelativelyPrimeInt(publicKey.N)
...
func (publicKey *PublicKey) HomoMult(m, c1 *big.Int) (*big.Int, error) {
	...
	// cipher^m mod N2
	return prime.ModInt(N2).Exp(c1, m), nil
}
...
func (publicKey *PublicKey) HomoAdd(c1, c2 *big.Int) (*big.Int, error) {
	...
	// c1 * c2 mod N2
	return prime.ModInt(N2).Mul(c1, c2), nil
}
...
func (privateKey *PrivateKey) Decrypt(c *big.Int) (m *big.Int, err error) {
	...
	m = prime.ModInt(privateKey.N).Mul(Lc, inv)
	return
}
...
func (privateKey *PrivateKey) Proof(k *big.Int, ecdsaPub *curve.ECPoint) Proof {
	var pi Proof
	iters := ProofIters
...
func (pf Proof) Verify(pkN, k *big.Int, ecdsaPub *curve.ECPoint) (bool, error) {
	iters := ProofIters
...
func GenerateXs(m int, k, N *big.Int, ecdsaPub *curve.ECPoint) []*big.Int {
	var i, n int
	ret := make([]*big.Int, m)
	...
	for i < m {
		...
		for j := 0; j < blocks; j++ {
			go func(j int) {
				...
				hash := cmt.SHA512_256(ib, jBz, nb, kb, sXb, sYb, Nb)
				chs[j] <- hash
			}(j)
		}
		...
		if curve.IsNumberInMultiplicativeGroup(N, ret[i]) {
			i++
		} else {
			n++
		}
